  ...
  return (
    <div className="space-y-6 font-urbanist">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-[#111111] rounded-2xl p-5 border border-slate-200/90 dark:border-slate-800 shadow-sm">
          <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Authorized Admin Actions</span>
        ...
        <div className="bg-white dark:bg-[#111111] rounded-2xl p-5 border border-slate-200/90 dark:border-slate-800 shadow-sm">
          <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Protected Media Resources</span>
          <div className="mt-2 flex items-baseline justify-between">
        ...
        <div className="bg-white dark:bg-[#111111] rounded-2xl p-5 border border-slate-200/90 dark:border-slate-800 shadow-sm">
          <span className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Security & Transcode Alerts</span>
          <div className="mt-2 flex items-baseline justify-between">
      ...
      <div className="bg-white dark:bg-[#111111] rounded-2xl p-6 border border-slate-200/90 dark:border-slate-800 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <div>
            <h3 className="font-bold text-slate-950 dark:text-white text-base">Administrative Action Density</h3>
            <p className="text-xs text-slate-400 dark:text-slate-500">Hourly mutation frequency across content and user models</p>
          </div>
        ...
        {/* Histogram Bars */}
        <div className="h-24 flex items-end justify-between gap-1 pt-4 border-t border-slate-100 dark:border-slate-800">
          {histogramBars.map((height, i) => (
            <div key={i} className="flex-1 flex flex-col items-center group relative cursor-pointer">
              <div
                className="w-full bg-slate-200 dark:bg-slate-700 group-hover:bg-[#FEF08A] dark:group-hover:bg-[#FEF08A] rounded-t transition-colors"
      ...
      <div className="bg-white dark:bg-[#111111] rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50 dark:bg-[#161616]">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-8 py-2 text-xs font-semibold bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl focus:border-[#FEF08A] focus:outline-none"
            />
            {searchTerm && (
              <button
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium">
              {filteredLogs.length === 0 ? (
                ...
                <tr key={log.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="py-3 px-4 font-mono text-slate-400 dark:text-slate-500 text-[11px]">
                    {log.duration}
                  </td>
                  <td className="py-3 px-4 text-right">
                    <button
                      onClick={() => setSelectedDiff(log)}
                      className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-[#FEF08A] hover:text-black dark:hover:bg-[#FEF08A] dark:hover:text-black text-slate-800 dark:text-slate-200 rounded-lg font-bold text-xs transition-colors border border-slate-200 dark:border-slate-700"
                    >
                      View Diff
                    </button>
                  </td>
      ...
      {selectedDiff && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-950 text-white rounded-2xl w-full max-w-lg p-6 shadow-2xl border border-slate-800">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800 mb-4">
              <div>
                <span className="text-xs font-mono font-bold text-[#FEF08A]">{selectedDiff.action}</span>
                <h4 className="text-sm font-bold text-white mt-0.5">{selectedDiff.targetId}</h4>
              </div>
              <button onClick={() => setSelectedDiff(null)} className="p-1 text-slate-400 hover:text-white rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs font-mono">
              <div>
                <p className="text-[10px] font-bold text-slate-400 mb-1">MUTATION DIFF (BEFORE / AFTER):</p>
                <div className="bg-slate-900 p-3 rounded-xl border border-slate-800 overflow-x-auto text-[11px]">
